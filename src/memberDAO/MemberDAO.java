...
public class MemberDAO
{
	...
	private static ArrayList<String> countListSQL = new ArrayList<String>();							// count 전용 sql문
	private static ArrayList<String> sqlList = new ArrayList<String>();								// 선택범위용 sql문
	...
	public void updatePetDel(String id, ArrayList<PetDTO> petList)
	{
		try
		{
			...
			conn = ConnectionDAO.getConnection();
			String sql = "select * from pet where id=?";					// 해당 id의 pet table정보를 검색한다.
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
			while(rs.next())
			{
				...
				p_num.setNum(rs.getInt("num"));								// DB에서 해당 id를 검색한 반려동물의 num(고유번호)을 저장 및 호출
				int petNumDB = p_num.getNum();
				for(int i=0; i<petList.size(); i++)							// for문으로 수정한 반려동물 동적 테이블의 행의 수까지 반복한다.
				{
					PetDTO p_dto = new PetDTO();							
					p_dto = petList.get(i);
					int petNumUser = p_dto.getNum();						// DB에 검색된 반려동물의 고유번호와 회원이 수정한 반려동물의 고유번호를 비교한다.
					if((petNumUser != 0) && (petNumUser == petNumDB))
					{	count++;	}										// 비교한 결과, DB에 검색된 반려동물의 고유번호와 회원이 수정한 반려동물의 고유번호가 같으면 count를 1추가한다.
				}
				if(count == 0)												// count가 0인 반려동물의 정보를 삭제한다.
				{	
	...
	public int getMemberCount(String col, String searchMember, int countSearch) throws Exception
	{
		...
		//	{
		//		search = select2+range+where;		// 첫번째 범위
		//		result = select+search;				// 첫번째 검색
		//	}
		
			if(countSearch >= searchList.size())
			{
				for(int i=0; i<countSearch; i++)			// col과 searchMember 검색 항목 저장
				{	
	...
	{ 
		...
		try 
		{
			...
			String search = "";				
			String result = ""; 		// 1번째 sql문
			ArrayList<MemberDTO> searchList = new ArrayList<MemberDTO>();
					
		//	if(countSearch > 1)
		//	{
		//		search = select2+sqlList.get(sqlList.size()-1)+whereSel;		// n번째 범위
		//		result = select+search+rowOrder+where;
		//	}
		//	else if(countSearch == 1)
		//	{
		//		search = select2+range+whereSel;				// 1번째 범위
		//		result = select+search+rowOrder+where;							// 1번째 검색
		//	}
