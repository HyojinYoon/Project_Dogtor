...
<%
	...
	doctorDTO dto = docDao.getDoctor(id);
%>
...
<script type="text/javascript">
		...
		function findList(i,m){
			...
			if(m == todayMon){
				if(i >= todayDate){
					window.open("doctorConfirmSchedulePro.jsp?code="+code+"&reserve_date="+year+-+month+-+date,"_blank");
				}
			}
			//이번달 이후 경우
			if(m > todayMon){
...
<body>
  ...
  <div id="topbar" class="d-none d-lg-flex align-items-center fixed-top">
    <div class="container d-flex">
      <div class="contact-info mr-auto">
        ...
        <%if(id != null){%>
        <i class="icofont-envelope"></i> <%=dto.getName()%>[<%=dto.getCode()%>]님 안녕하세요.
        <%}%>
      </div>
    </div>
  </div>
  ...
  <!-- 여기에 본인 파트 시작 -->
    ...
    <p></p>
    <center><img src="/dogtor/save/breaktime.png" width="352" /></center>
<table id="calendar" border="1" align="center" style="border-color:#9594a2 ">
	...
	<br /><br />
	<%--진료확인 달력으로 이동 --%>
	<center><h3>진료를 확인하고 싶다면?</h3>
	<form action="doctorConfirmCalanderR.jsp" method="post" onSubmit="goMain()">
		<input type="hidden" name="code" value="<%=code%>" />
		<input type="submit" value="진료확인"  /></center>
	</form>
	</div>
	<br /><br />
