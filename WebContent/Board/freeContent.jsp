...
<%
	...
	String adminNick = dao.getAdminNick(status, id);

	SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
   
	DAO dbPro = new DAO();
	//DTO article =  dbPro.getArticle(num);
	DTO article =  dbPro.getArticle(num, nick);
	datDAO datDao =  new datDAO();
	
	List articleList = null;
	int count = datDao.getArticleCountDat(num);
	if (count > 0) {
	   articleList = datDao.getDatArticles(num);
	}
%>
<script type = "text/javascript">
		//댓글 공백시 벨류체크
        function valueCheck(){
        	var contentValue = document.writeform.contentDat.value;
      ...
      //댓글 수정시 벨류체크
        function valueCheck2(){
        	var contentValue = document.reply.contentDat.value;
        ...
        var con ="";
		function datUpdate(datId,content,num,superNum){
			document.getElementById('data'+datId).style.display='none';
			document.getElementById('dat'+datId).innerHTML="<form action='datUpdatePro.jsp'>"+
												"<input type='hidden' name='superNum' value='"+superNum+"' />"+
												"<input type='hidden' name='num' value='"+num+"' />"+
												"<input type='hidden' name='pageNum' value='<%=pageNum%>' />"+
												"<textarea name='contentDat'>"+content+"</textarea><br/>"+
												"비밀번호 <input type='password' name='passwd' /> <br/>"+
												"<input type='submit' value='수정하기' />"+
												"<input type='button' value='취 소' onclick=datC("+datId+",'"+content+"') />"+
												"</form>";		
		}
		
		function datUpdate2(datId,num,superNum){
			document.getElementById('data'+datId).style.display='none';
			con = document.getElementById('dat'+datId).innerHTML;
			//수정사항
			document.getElementById('dat'+datId).innerHTML="<form name='reply' action='datUpdatePro.jsp' onsubmit='return valueCheck2();'>"+
												"<input type='hidden' name='superNum' value='"+superNum+"' />"+
												"<input type='hidden' name='num' value='"+num+"' />"+
												"<input type='hidden' name='pageNum' value='<%=pageNum%>' />"+
												"<textarea name='contentDat'>"+con+"</textarea><br/>"+
												"비밀번호 <input type='password' name='passwd' /> <br/>"+
												"<input type='submit' value='수정하기' />"+
												"<input type='button' value='취 소' onclick=datU('"+datId+"') />"+
												"</form>";		
		}
		
		function datU(datId){			
			document.getElementById('data'+datId).style.display='block';
			document.getElementById('dat'+datId).innerHTML = con;
		}
		
		function datC(datId){			
			document.getElementById('data'+datId).style.display='block';
			document.getElementById('datc'+datId).style.display='none';
		}
		
		function datDelete(datId,num,superNum,content){
			document.getElementById('data'+datId).style.display='none';
			document.getElementById('dat'+datId).innerHTML="<form action='datDeletePro.jsp'>"+
			"<input type='hidden' name='superNum' value='"+superNum+"' />"+
			"<input type='hidden' name='num' value='"+num+"' />"+
			"<input type='hidden' name='pageNum' value='<%=pageNum%>'/>"+
		...
		function datDelete2(datId,num,superNum){
			document.getElementById('data'+datId).style.display='none';
			document.getElementById('datc'+datId).style.display='block';	
		}
</script>
...
<table width="1000" border="1" cellspacing="0" cellpadding="0"  align="center">  
	<tr height="30">
		<td align="center" width="250" >글번호</td>
		<td align="center" width="250" align="center"><%=article.getNum()%></td>
	    <td align="center" width="250" >조회수</td>
	    <td align="center" width="250" align="center"><%=article.getReadcount()%></td>
  </tr>
  <tr height="30">
	    <td align="center" width="250" >작성자</td>
	    <td align="center" width="250" align="center"><%=article.getWriter()%></td>
	    <td align="center" width="250"  >작성일</td>
	    <td align="center" width="250" align="center"><%= sdf.format(article.getReg())%></td>
  </tr>
  <tr height="30">
		<td align="center" width="250" >글제목</td>
		<td align="center" width="750" align="center" colspan="3"><%=article.getHead()%><%=article.getSubject()%></td>
  </tr>
  <tr>
	    <td align="center" width="250" >글내용</td>
	    <td align="left" width="750" colspan="3"><pre><%=article.getContent()%></pre></td>
  </tr>
  <%if(article.getSave() != null) {%>
  <tr>
	    <td align="center" width="250" >첨부파일</td>
	    <td align="left" width="750" colspan="3"><pre></pre>
	        <img  src="/dogtor/save/<%=article.getSave() %>"  style="max-width:1024px; max-height:800px; width:100%; height:100%;"/>
	    </td>
  </tr>

  <%} %>
   <form method="post" name="writeform" action="datWritePro.jsp" onsubmit="return valueCheck()"; >
   <input type="hidden"  name="superNum" value="<%=num %>"/>
   <input type="hidden"  name="pageNum" value="<%=pageNum %>"/>
   <tr >
        <td align="center" width="250" >댓글</td>
        <td  colspan="3"> 
        <%if(status != 0){ %>
        <textarea name="contentDat" rows="3" cols="40"  placeholder="댓글을 작성하세요" ></textarea> 
       ...
       <%if(count !=0){
            for (int i = 0 ; i < articleList.size() ; i++) {  //articleList는 각 게시물 list에 담아둔것
            	datDTO dto = new datDTO();
        	     dto = (datDTO)articleList.get(i); %>
     
				<tr> 
          	    	<td><%=dto.getWriter()%>&nbsp; 날짜: <%=dto.getReg_date() %></td>
				</tr>
				<tr>
					<td>
						<div id="dat<%=i%>"><%=dto.getContentDat() %></div>
						<div id="datc<%=i%>" style="display:none;">
							 <form action='datDeletePro.jsp'>
								<input type='hidden' name='superNum' value='<%=dto.getSuperNum()%>' />
								<input type='hidden' name='num' value='<%=dto.getNum()%>' />
								<input type='hidden' name='pageNum' value='<%=pageNum%>'/>
								<input type='submit' value='삭제하기' />
								<input type='button' value='취 소' onclick=datC('<%=i%>') />
							</form>
						</div>
						<br />
						<%if(status==1 && nick.equals(dto.getWriter())){ %>
						<div id="data<%=i%>">
							<input type="button" value="댓글 수정" onclick="datUpdate2('<%=i%>','<%=dto.getNum()%>','<%=num %>');" />
                        	<input type="button" value="댓글 삭제" onclick="datDelete2('<%=i%>','<%=dto.getNum()%>','<%=dto.getSuperNum()%>');"/>
                        </div>
                        <%}else if (status==100 && nick.equals(dto.getWriter())){%>
                        <div id="data<%=i%>">
                            <input type="button" value="댓글 수정" onclick="datUpdate2('<%=i%>','<%=dto.getNum()%>','<%=num %>');" />
                        	<input type="button" value="댓글 삭제" onclick="datDelete2('<%=i%>','<%=dto.getNum()%>','<%=dto.getSuperNum()%>');"/>
                        </div>   
  ...
  <tr height="30">      
	    <td colspan="4"  align="right" >
	      ...
	      <% 
	    	if(status == 1 && article.getWriter().equals(nick)){ %>
				<input type="button" value="글수정" onclick="window.location='freeUpdateForm.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">
				<input type="button" value="글삭제" onclick="window.location='freeDeleteForm.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">
				<input type="button" value="자유게시판 글목록" onclick="window.location='freeList.jsp?pageNum=<%=pageNum%>'">
		  <%}else if(status == 100){%>
			<input type="button" value="글삭제" onclick="window.location='freeDeleteFormAdmin.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">
			<input type="button" value="자유게시판 글목록" onclick="window.location='freeList.jsp?pageNum=<%=pageNum%>'">
			<input type="hidden" name="num" value="<%=num%>">
			    <%if(article.getWriter().equals(adminNick)){ %>
			           <input type="button" value="글수정" onclick="window.location='freeUpdateForm.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>&writer=<%=url%>'">
