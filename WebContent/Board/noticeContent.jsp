...
<table width="500" border="1" cellspacing="0" cellpadding="0"  align="center">  
  ...
  <tr height="30">
		...
		<td align="center" width="375" align="center" colspan="3"><%=article.getHead()%><%=article.getSubject()%></td>
  </tr>
  <tr>
  ...
  <tr height="30">      
	    <td colspan="4"   align="right" >
	    	...
	    	<%
	    	if(session.getAttribute("memStatus") != null){ 
	    	status = (Integer)session.getAttribute("memStatus") ;
	    	}
	    	if(status == 100){ %>
				<input type="button" value="글수정" onclick="window.location='noticeUpdateForm.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">
				<input type="button" value="글삭제" onclick="window.location='noticeDeleteForm.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">
				<%}%>
			<input type="button" value="글목록" onclick="window.location='noticeList.jsp?pageNum=<%=pageNum%>'">
	    </td>
  </tr>
</table>
<br /><br />
