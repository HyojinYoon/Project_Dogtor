...
<meta charset="utf-8">
  ...
  <link href="assets/css/style.css" rel="stylesheet">
    <!-- 스크립트 및 자바 정리해주세요. -->
   <%int pageSize = 10; // 한페이지 보여질 게시물 수.
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm"); // 작성된 날짜를 해당 형식으로 보기위함. 

    String pageNum = request.getParameter("pageNum"); // 리스트에서 페이지 번호 클릭시 받을 수 있다.
    if (pageNum == null) { //주어진 pageNum이 없다면 1 부여
        pageNum = "1";
    }

    int currentPage = Integer.parseInt(pageNum); // 숫자 1
    ...
    DAO dbPro =new DAO();
    count = dbPro.getFreeArticleCount();
    if (count > 0) {
        articleList = dbPro.getFreeArticles(startRow, endRow);
    }
    
	number=count-(currentPage-1)*pageSize;
%>

<script  type = "text/javascript">  <%-- 검색대상 별 입력창 혹은 select문이 나오게끔 하는 스크립트 --%>
     
    function select(s){
    	var colValue = s;
    	
    	if(colValue == "head"){
    		document.getElementById('result').innerHTML="<select name='search'>"+ 
                "<option value='[병원리뷰]'>[병원리뷰]</option>"+
                "<option value='[건강/사료]'>[건강/사료]</option>"+
                "<option value='[추천용품]'>[추천용품]</option>"+
                "<option value='[자유글]'>[자유글]</option>"+
            "</select>"
        }
    	if(colValue == "writer" || colValue == "subject"){
    		document.getElementById('result').innerHTML="<input type='text' name='search' />";
        }	
    }
</script>
  <!-- 스크립트 및 자바 정리 종료. -->
...
<div class="container" data-aos="fade-up">
  ...
  <div class="section-title">
          <h2>게시판</h2>
          <h3><span>자유게시판</span>(전체 글:<%=count%>)</h3>
          <p></p>
        </div>
<center>
<table width="700">
	<tr>
    	...
    	<%if(status == 100 || status==1) { %>
    	    <a href="freeMyList.jsp?pageNum=<%=pageNum%>">[나의 작성글 보기]&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>
    		<a href="freeWriteForm.jsp?pageNum=<%=pageNum%>">글쓰기</a>
        <%}else{ %>
...
<%}else{
    	...
    	int datCount = datDao.getDatCount(num);
%>
	<tr height="30">
    	...
    	<td  width="250" >
		  <%int wid=0;%>
	      <%=article.getHead()%>
     	  <a href="freeContent.jsp?num=<%=article.getNum()%>&pageNum=<%=currentPage%>">
		  <%=article.getSubject()%>
          </a> 
          <%if(article.getSave() != null){
                    wid=20;%>
					<img src="images/save.JPG" width="<%=wid%>" height="16">
		     <%}%>
...
<%
    if (count > 0) {
        ...
        if (startPage > 10) {    %>
        <a href="freeList.jsp?pageNum=<%= startPage - 10 %>">[이전]</a>
<%      }
        for (int i = startPage ; i <= endPage ; i++) {  %>
        	<a href="freeList.jsp?pageNum=<%= i %>">[<%= i %>]</a>
<%		}
        if (endPage < pageCount) {  %>
        	<a href="freeList.jsp?pageNum=<%= startPage + 10 %>">[다음]</a>
<%		}
    }%>
<%if(status == 100 || status==1) { %>
            <a href="freeMyList.jsp?pageNum=<%=pageNum%>">[나의 작성글 보기]&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>
...
<%}%>
<br/>
<form action = "freeSearchList.jsp?pageNum=<%=pageNum%>" method = "post">
<%--<form action = "freeSearchList.jsp?pageNum=<%=pageNum%>&countSearch=<%=countSearch%>" method = "post"> --%>
	
	<select name = "col"  id="col"  onchange="select(this.value)"  > 
