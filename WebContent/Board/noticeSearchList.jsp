...
<%
	...
	//countSearch 검색 횟수 받아오기.
	//int countSearch = Integer.parseInt(request.getParameter("countSearch"));
	
    int pageSize = 10; // 한페이지 보여질 게시물 수.
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm"); // 작성된 날짜를 해당 형식으로 보기위함. 91라인에서 사용중.
...
//  count = dbPro.getArticleCount(col, search);
    count = dbPro.getNoticeArticleSimilarityCount(col, search);	//연관검색
    if (count > 0) {
    	//재검색, 게시글 받아오기
    	//countSearch++;
 //       articleList = dbPro.getNoticeArticles(col, search, startRow, endRow);
		articleList = dbPro.getNoticeArticlesSimilarity(col, search, startRow, endRow);	//연관검색
    }
    
	number=count-(currentPage-1)*pageSize;
	id = (String)session.getAttribute("memId");      //세션id
...
<a href = "">예약</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a href = "">예약확인</a><br/>
<a href = "">의료서비스</a>&nbsp;&nbsp;<a href = "">게시판</a href = "">&nbsp;&nbsp;<a href="/dogtor/Member/memberForm/mypageForm.jsp">마이페이지</a>
<hr>
<a href = "noticeList.jsp">공지사항</a>&nbsp;&nbsp;<a href = "list2.jsp">문의게시판</a>&nbsp;&nbsp;<a href = "freeList.jsp">자유게시판</a>
<hr>
...
<body>
 ...
 <div class="container" data-aos="fade-up">
	<div class="section-title">   
       <h2>게시판</h2>     
       <h3><span>공지게시판 검색 목록</span>(전체 글:<%=count%>)</h3>        
	</div>
...
<table width="700">
	<tr>
    	<td align="right" >
    	<%if(status == 100) { %>
    		<a href="noticeWriteForm.jsp">글쓰기(관리자)</a>
    		<%} %>
    	</td>
    </tr>
</table>
...
<%
    if (count > 0) {
        ...
        if (startPage > 10) {    %>
        <a href="noticeSearchList.jsp?pageNum=<%= startPage - 10 %>&col=<%=col%>&search=<%=search%>">[이전]</a>
<%      }
        for (int i = startPage ; i <= endPage ; i++) {  %>
        	<a href="noticeSearchList.jsp?pageNum=<%=i%>&col=<%=col%>&search=<%=search%>">[<%= i %>]</a>
<%		}
        if (endPage < pageCount) {  %>
        	<a href="noticeSearchList.jsp?pageNum=<%= startPage + 10 %>&col=<%=col%>&search=<%=search%>">[다음]</a>
<%		}
    }
%>
</center>
<center>
<form action = "noticeSearchList.jsp" method = "post">
	<select name = "col"> 
		<option value = "subject">제목</option>
		<option value = "writer">작성자</option>
	</select>
	<input type = "text" name = "search" />
	<%--<input type = "hidden" name = "countSearch" value="<%=countSearch%>" />--%>
