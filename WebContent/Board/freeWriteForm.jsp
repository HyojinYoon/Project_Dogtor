...
<script>
	function addRow()
	{
		var picture_table = document.getElementById("picture_tbody");		// 행을 추가할 테이블명
		var picture_row = picture_table.insertRow(picture_table.rows.length)	// 추가할 행

		// 실제 행 추가 (숫자는 컬럼의 수)
		var cell1 = picture_row.insertCell(0);
		var cell2 = picture_row.insertCell(1);
		
		// 추가할 행에 원하는 요소 추가
		cell1.innerHTML = " <td><input type='checkbox' name='check' style='width:35px;height:15px' /></td>";
		cell2.innerHTML = " <td><input type='file' class='font-control' name='save' /> <br /></td> ";
	}
	
	function delRow()
	{
		var picture_table = document.getElementById("picture_tbody");		// 행을 삭제할 테이블명
		for(var i=0; i<picture_table.rows.length; i++)
		{
			var delcheck = picture_table.rows[i].cells[0].childNodes[0].checked;
			if(delcheck)
			{
				picture_table.deleteRow(i);
				i--;
			}
		}
	}
...
<br>
<form method="post" name="writeform" action="freeWritePro.jsp" onsubmit="return writeSave()" enctype="multipart/form-data">
...
<table width="400" border="1" cellspacing="0" cellpadding="0"   align="center">
   <tr>
    <td align="right" colspan="2" >
       <a href="freeList.jsp?pageNum=<%=pageNum%>"> 자유게시판 글목록</a> 
   </td>
   </tr>
   <tr>
   ...
   <tr>
    <td  width="70"   align="center">말머리</td>
    <td> <select name="head">
  ...
  <tr>
    <td  width="70"   align="center" >첨 부 파 일</td>
    <td  width="330" >
     <input type="file"  name="save"> 
    <%-- 사진 3개용 <input type="file"  name="save1">
  ...
  <%-- <tr>
      ...
      <input type="button" name="minus" value="-" onclick="delRow();"/>
      </td>
      <td width="70"   align="center">첨 부 파 일</td>
  </tr>
  ...
  <tr>  
    <td  width="330" colspan=2>
    	...
    	<tbody id="picture_tbody">
    		<tr>
    			 <td><input type="checkbox" name="check" style="width:35px;height:15px" /></td>
    			 <td><input type="file" class="font-control" name="save" /> <br /></td>   
    		</tr>
