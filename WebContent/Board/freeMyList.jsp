...
<%
    ...
    DAO dbPro =new DAO();
    count = dbPro.getArticleCountFree(nick);
    
    if (count > 0) {
        articleList = dbPro.getArticlesFree(nick, startRow, endRow);
    }
...
<div class="section-title">   
       <h2>게시판</h2>     
       <h3><span>나의 작성글 목록</span>(전체 글:<%=count%>)</h3>        
</div>
<table width="700">
	<tr>
    	<td align="right">
    	...
    	if(id != null){
    	%>
    		<a href="freeList.jsp?pageNum=<%=pageNum%>">자유게시판 글 목록</a>
        <%
        }else{
        %>
...
<%
    if (count > 0) {
        ...
        if (startPage > 10) {    %>
        <a href="freeMyList.jsp?myPageNum=<%= startPage - 10 %>&pageNum=<%=pageNum%>">[이전]</a>
<%      }
        for (int i = startPage ; i <= endPage ; i++) {  %>
        	<a href="freeMyList.jsp?myPageNum=<%=i%>&pageNum=<%=pageNum%>">[<%= i %>]</a>
<%		}
        if (endPage < pageCount) {  %>
        	<a href="freeMyList.jsp?myPageNum=<%= startPage + 10 %>&pageNum=<%=pageNum%>">[다음]</a>
<%		}%>
      <%} %> 
</center><br/><br/></div>
