...
<%
    ...
    DAO dbPro =new DAO();
    count = dbPro.getArticleCountQna(nick);
    if (count > 0) {
        articleList = dbPro.getArticlesQna(nick, startRow, endRow);
    }
...
<table width="700">
	<tr>
    	...
    	if(id != null){
    	%>
    		<a href="list2.jsp?pageNum=<%=pageNum%>">문의게시판 글 목록</a>
        <%
        }else{
        %>
...
<%
    if (count > 0) {
        ...
        if (startPage > 10) {    %>
        <a href="myListQna.jsp?myPageNum=<%= startPage - 10 %>&pageNum=<%=pageNum%>">[이전]</a>
<%      }
        for (int i = startPage ; i <= endPage ; i++) {  %>
        	<a href="myListQna.jsp?myPageNum=<%=i%>&pageNum=<%=pageNum%>">[<%= i %>]</a>
<%		}
        if (endPage < pageCount) {  %>
        	<a href="myListQna.jsp?myPageNum=<%= startPage + 10 %>&pageNum=<%=pageNum%>">[다음]</a>
<%		}%>
      <%} %> 
</center><br/><br/></div>
