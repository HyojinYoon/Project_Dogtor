...
<%//세션검사 지우면 안됩니다. 헤더에서 사용중입니다. 만약 본인 내용에 세션 id 정검이 있을 경우 그 내용을 지워주세요.
	...
	if(status == null){
		status = 0;
		response.sendRedirect("/dogtor/Member/memberForm/loginForm.jsp");
	}
	int num = Integer.parseInt(request.getParameter("num"));
	DAO dao = new DAO();
	DTO dto = dao.getArticleCheck(num);
    String check = dao.getAdminNick(status, id); //로그인 아이디와 작성자 닉네임이 같은지 확인
    if(status!=100){
        if(!dto.getWriter().equals(check)){%>
	        <script>
	              alert("해당 글에 대한 열람 권한이 없어 로그인창으로 이동합니다.");
...
<%
	...
	//int num = Integer.parseInt(request.getParameter("num"));  //리스트에서 해당 게시글 눌렀을때, 고유번호 받기
	String pageNum = request.getParameter("pageNum");         //리스트의 페이지번호 받기 (글목록등으로 돌아갈때 기존페이지로 돌아가기 위하여)
	String writer = request.getParameter("writer");           //작성자 받기
	...
	DTO article =  dbPro.getArticle(num, nick); //num에 맞는 게시글 갖고오며 작성자가 열람할 경우에는 조회수 증가X
	String adminNick = dbPro.getAdminNick(status, id);  //id에 맞는 member테이블의 nick값 확인
	int ref=article.getRef();            //원글그룹 번호
	int re_step=article.getRe_step();    //원글그룹내의 작성순서
	int re_level=article.getRe_level();  //원글그룹내의 댓글 그룹
...
<table width="500" border="1" cellspacing="0" cellpadding="0"   align="center">  
	<tr height="30">
		<td align="center" width="125">글번호</td>                   <%--자동 글번호 --%>
		<td align="center" width="125" align="center"><%=article.getNum()%></td>
	    <td align="center" width="125" >조회수</td>                   <%--조회수 --%>
	    <td align="center" width="125" align="center"><%=article.getReadcount()%></td>
  </tr>
  <tr height="30">
	    <td align="center" width="125" >작성자</td>                   <%--작성자 = 닉네임 --%>
	    <td align="center" width="125" align="center"><%=article.getWriter()%></td>
	    <td align="center" width="125" >작성일</td>
	    <td align="center" width="125" align="center"><%= sdf.format(article.getReg())%></td>   <%--작성자 날짜 --%>
  </tr>
  <tr height="30">
		<td align="center" width="125" >글제목</td>
		<td align="center" width="375" align="center" colspan="3"><%=article.getSubject()%></td>   <%--글 제목--%>
  </tr>
  <tr>
	    <td align="center" width="125" >글내용</td>                         
	    <td align="left" width="375" colspan="3"><pre><%=article.getContent()%></pre></td>        <%--글 내용--%>
  </tr>
  <tr height="30">      
	    ...
	    <%if(status == 1){%>  <%--회원이면 글수정, 삭제, 답글쓰기, 글목록 버튼 노출 후 각 페이지로 이동 --%>
	    	<input type="button" value="글수정" onclick="window.location='updateFormQna.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">
			<input type="button" value="글삭제" onclick="window.location='deleteFormQna.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>&ref=<%=ref%>&re_step=<%=re_step%>&re_level=<%=re_level%>'">
			<input type="button" value="답글쓰기" onclick="window.location='writeForm.jsp?num=<%=num%>&ref=<%=ref%>&re_step=<%=re_step%>&re_level=<%=re_level%>&writer=<%=url%>&pageNum=<%=pageNum%>'">
			<input type="button" value="글목록" onclick="window.location='list2.jsp?pageNum=<%=pageNum%>'">
			<input type="hidden" name="num" value="<%=num%>">
	    <%}else if(status == 100){%> <%--관리자면 전글에 대해 삭제, 답글쓰기, 글목록 버튼 노출 후 각 페이지로 이동 --%>
			<input type="button" value="글삭제" onclick="window.location='deleteFormAdmin.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>&ref=<%=ref%>&re_step=<%=re_step%>&re_level=<%=re_level%>'">
			<input type="button" value="답글쓰기" onclick="window.location='writeForm.jsp?num=<%=num%>&ref=<%=ref%>&re_step=<%=re_step%>&re_level=<%=re_level%>&writer=<%=url%>&pageNum=<%=pageNum%>'">
			<input type="button" value="글목록" onclick="window.location='list2.jsp?pageNum=<%=pageNum%>'">
			<input type="hidden" name="num" value="<%=num%>">
			    <%if(article.getWriter().equals(adminNick)){ %> <%--관리자가 본인글 들어갔을때는 글수정 버튼도 노출 --%>
			           <input type="button" value="글수정" onclick="window.location='updateFormQna.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>&writer=<%=url%>'">
