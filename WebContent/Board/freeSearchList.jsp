...
<%
    ...
    int currentPage = Integer.parseInt(searchPageNum); // 1
    ...
    DAO dbPro =new DAO();
 // count = dbPro.getArticleCountFree(col, search);
 	count = dbPro.getArticleCountFreeSimilarity(col, search);	// 연관검색
    if (count > 0) {
 //    articleList = dbPro.getArticlesFree(col, search, startRow, endRow);
       articleList = dbPro.getArticlesFreeSimilarity(col, search, startRow, endRow);	// 연관검색
    }
...
<div class="section-title">   
       <h2>게시판</h2>     
       <h3><span>자유게시판 검색 목록</span>(전체 글:<%=count%>)</h3>        
</div>
<table width="700">
	<tr>
    	<td align="right" >
    	
    		<a href="freeList.jsp?pageNum=<%=pageNum%>">자유게시판 목록</a>
...
<%
    if (count > 0) {
        ...
        if (startPage > 10) {    %>
        <a href="freeSearchList.jsp?searchPageNum=<%= startPage - 10 %>&pageNum=<%=pageNum%>&col=<%=col%>&search=<%=url %>">[이전]</a>
<%      }
        for (int i = startPage ; i <= endPage ; i++) {  %>
        	<a href="freeSearchList.jsp?searchPageNum=<%= i %>&pageNum=<%=pageNum%>&col=<%=col%>&search=<%=url %>">[<%= i %>]</a>
<%		}
        if (endPage < pageCount) {  %>
        	<a href="freeSearchList.jsp?searchPageNum=<%= startPage + 10 %>&pageNum=<%=pageNum%>&col=<%=col%>&search=<%=url %>">[다음]</a>
<%		}
    }
%>
