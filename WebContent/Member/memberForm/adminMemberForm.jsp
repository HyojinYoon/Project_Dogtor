...
<%
	...
	count = dao.getMemberCount();										// 16
	 if (count > 0)
	 {   memberList = dao.getMembers(startRow, endRow);	}
...
<%}else{ %>
	<table border="1" align="center" width="1300" cellpadding="0" cellspacing="0">
		<tr height="30"> 
			...
			<td align="center" width="100"><font size = "4em">PW</font></td>
...
<%
    if (count > 0) {
        ...
        if (startPage > 10) {    %>
        <a href="adminMemberForm.jsp?pageNum=<%= startPage - 10 %>">[이전]</a>
        <%}%>
        <%for (int i = startPage ; i <= endPage ; i++) {  %>
        	<a href="adminMemberForm.jsp?pageNum=<%= i %>">[<%= i %>]</a>
		<%}%>
		<%if (endPage < pageCount) {  %>
        	<a href="adminMemberForm.jsp?pageNum=<%= startPage + 10 %>">[다음]</a>
		<%}%>
        <%} %>
