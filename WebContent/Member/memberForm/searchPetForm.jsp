...
<%
	...
	String col = request.getParameter("col");					// 펫검색 테마
	String searchPet = request.getParameter("searchPet");		// 입력한 검색 호출
	...
	int count = 0;										// 검색 결과의 전체 게시물 수
	
	ArrayList<PetDTO> petCurrentList = null;
	MemberDAO p_dao = new MemberDAO();					// DAO 생성
	count = p_dao.getPetCount(col,searchPet);			// pet 테이블에서 검색한 전체 레코드 수 리턴
	if(count > 0)
	{	petCurrentList = p_dao.getPetList(col,searchPet,startRow,endRow);	}	// startRow부터 endRow까지의 해당 pet정보를 얻는 메서드(9개)
		...
		<td align="right"" cellpadding="0" cellspacing="0">	
			<%if(status == 100 )
			  {%>	
			  	  ...
			  	  <a href="adminMemberForm.jsp">회원등급변경</a> &emsp;
...
<%
  ...
  <%}
	...
	<%
		for(int i=0; i < petCurrentList.size(); i++)		// 검색된 펫 데이터 생성
 ...
 <%	// 페이지 번호 나타내기
	if(count > 0)
	{
		...
		if(startPage > 10)
		{%> <a href="searchPetForm.jsp?pageNum=<%=startPage - 10 %>&col=<%=col%>&searchPet=<%=searchPet%>">[이전]</a> <%}
		
		for(int i=startPage; i <= endPage; i++)
		{%>	<a href="searchPetForm.jsp?pageNum=<%=i %>&col=<%=col%>&searchPet=<%=searchPet%>">[<%=i %>]</a>	<%}
		
		if(endPage < pageCount)
		{%> <a href="searchPetForm.jsp?pageNum=<%=startPage+10 %>&col=<%=col%>&searchPet=<%=searchPet%>">[다음]</a>	<%}
	}%>
