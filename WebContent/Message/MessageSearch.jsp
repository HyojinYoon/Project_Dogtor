...
<%
   ...
   doctorDTO dto = doc.getDoctor(id);
   count = dao.getMessageCount(id, col, search);
   if (count > 0) {
       messageList = dao.getMessageArticles(id, col, search, startRow, endRow);
   }
      ...
      <!-- 진료확인하기 버튼 -->
        <%if(id == null){%>
          ...
          <%}else{%>
               ...
               <a href="/dogtor/Message/SendMessage.jsp" class="btn-get-started scrollto">보낸 메세지 함</a>
             <%}%>  
      </div>
    </div>
  </section><!-- End Hero -->
...
{%>
   ...
   {%>
       <center><div class="section-title"><h2> 안읽은 메세지 개수:<%=dao.getMyMessageCount(id)%></h2></div>
      <br />
         <%if (count == 0) {%>
      <table><tr><%="받은 메세지가 없습니다."%></tr></table>
         <%}else{%>
            <table border="1">
               <b><tr align = "center" bgcolor="#e2eefd"><td>번호</td><td width=250>제목</td><td width=150>보낸 사람</td><td width=200>보낸 날짜</td><td>읽음 여부</td></tr></b>
                  ...
                  <%for (int i = 0; i < messageList.size() ; i++){
                     ...
                     <tr>
                        ...
                        <td align = "center"><a href = "Content.jsp?num=<%=message.getNum()%>"><%if(message.getReadcheck()>=1){%><font color=black><%=message.getSubject()%></font><%}else{%><b><%=message.getSubject()%></b><%}%></a></td>
                        <td align = "center"><%=message.getWriter()%></td>
                        <td align = "center"><%=sdf.format(message.getRe_date())%></td>
                        <td align = "center"><%if(message.getReadcheck()>=1){%>읽음<%}else{%><font color=salmon>안읽음</font><%}%></td> 
					</tr>
				<%}%>
			</table>
...
<%
    if (count > 0){
        ...
        if (startPage > 10) 
        {%>
           <a href="MessageSearch.jsp?pageNum=<%= startPage - 10 %>&col=<%=col%>&search=<%=search%>"></a>[이전]
         <%}
        for (int i = startPage ; i <= endPage ; i++) 
        {  %> <a href="MessageSearch.jsp?pageNum=<%= i %>&col=<%=col%>&search=<%=search%>">[<%= i %>]</a> <%}
        if (endPage < pageCount){%>
            <a href="MessageSearch.jsp?pageNum=<%= startPage + 10 %>&col=<%=col%>&search=<%=search%>">[다음]</a>	
            <%}}}}%>

<form action="/dogtor/Message/MessageSearch.jsp" method="post">
   <select name="col">
