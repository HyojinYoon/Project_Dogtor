...
<%
	...
	count = dao.getMessageCount(col, search);
	if (count > 0) {
    	messageList = dao.getMessageArticles(col, search, startRow, endRow);
	}
...
<%
    if (count > 0) {
        ...
        if (startPage > 10) {%>
           <a href="SendMessageSearch.jsp?pageNum=<%= startPage - 10 %>&col=<%=col%>&search=<%=search%>">[이전]</a>
         <%}
        for (int i = startPage ; i <= endPage ; i++) {  %>
            <a href="SendMessageSearch.jsp?pageNum=<%= i %>&col=<%=col%>&search=<%=search%>">[<%= i %>]</a><%}
        if (endPage < pageCount){%>
            <a href="SendMessageSearch.jsp?pageNum=<%= startPage + 10 %>&col=<%=col%>&search=<%=search%>">[다음]</a>
         <%}}}}%>

<form action="SendMessageSearch.jsp" method="post">
   <select name="col">
