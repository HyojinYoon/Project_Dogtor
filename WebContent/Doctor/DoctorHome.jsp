...
<%
	...
	int m_count=0;
	m_count = dao2.getMessageCount(id);
	 ...
	 //id, code 세션 정보 가져옴.
	 	...
	 	String code = (String)session.getAttribute("docCode"); 
	 
	 	
		//오늘 날짜 받아와서 String 으로 저장
		LocalDate today = LocalDate.now();	
		String year = today.getYear()+"";
		String month = today.getMonthValue()+"";
		String day = today.getDayOfMonth()+"";
		
		//오늘 날짜 2021-5-21 형식으로 저장
	    ...
	    if(id != null) {
	    if(!id.equals("admin")){//의사일 경우
	 		//code를 기준으로 데이터 베이스에 있는 게시글 수를 받아온다.
	    	count = reserveDao.getDoctorReserveCount(code,date); 
	    	//code를 기준으로 게시글이 하나라도 있으면, 해당 게시글을 ArrayList로 받아온다.(게시글 정보 받아오기)
	    	if (count > 0){
	      		reserveList = reserveDao.getDoctorReserves(startRow, endRow, code, date);
	    	}
			number=count-(currentPage-1)*pageSize; //다른 페이지 때문에 계산해 주는것.
	    }if(id.equals("admin")){
	    	//날짜를 기준으로 모든 진료 수를 받아온다.
	    	count = reserveDao.getAllDoctorReserveCount(date); 
	    	//code를 기준으로 게시글이 하나라도 있으면, 해당 게시글을 ArrayList로 받아온다.(게시글 정보 받아오기)
	    	if (count > 0){
	      		reserveList = reserveDao.getAllDoctorReserves(startRow, endRow, date);
	    	}
			number=count-(currentPage-1)*pageSize; //다른 페이지 때문에 계산해 주는것.
	    }
	    }
...
<body>
  ...
  <section id="hero" class="d-flex align-items-center">
    <div class="container" data-aos="zoom-out" data-aos-delay="100">
 		...
 		<%if(id != null){%>
 		<h3><a href="/dogtor/Message/Message.jsp">안읽은 메세지 개수:<%=dao2.getMyMessageCount(id)%></a></h3>
 			<br />
         <%if (m_count == 0) {%>
      	<h3><%="받은 메세지가 없습니다."%></h3><%}}%>
  ...
  <!-- 여기에 본인 파트 시작 -->
  <%//의사 로그인시 해당 의사 당일 진료 내용 표기
	if(id != null){
		%>
		<body>
		
		<center><div class="section-title"><h2>오늘의 진료 일정(일정 수:<%=count%>)</b></h2></div> <%-- <center> 전체 가운데 정렬 --%>
		...
		<%  } else {%>
		<table border="1" width="1250" cellpadding="0" cellspacing="0" align="center">
			
			<tr height="30" bgcolor="#e2eefd" >
				<td align="center"  width="150"  >담당 선생님</td>
				<td align="center"  width="350" >진료 내용</td>
		...
		%>
			<tr height="30">             <%--number 감소하고 있음. --%>
		    	...
		    	<td align="center" width="300" ><%--진료 내용 --%>
		    			<%=reserve.getSymptom()%>
		    	</td>
		...
		<%
		    ...
		    if (count > 0) { //게시글이 하나라도 있으면,
		      ...
		      //11페이지 이상 넘어갈 경우 다시 1부터 시작하고 [이전] 표시를 출력한다. 이전 누르면 다시 1페이지 부터 10페이지 까지의 항목이 보인다.
		        ...
		        if (startPage > 10) {    %>
		        <a href="DoctorHome.jsp?pageNum=<%= startPage - 10 %>">[이전]</a>
		<%      }
		        //게시글이 하나여도 1부터 시작,
		        for (int i = startPage ; i <= endPage ; i++) {  %> <%--for문 반복 list페이지에 pageNum 전달한다.--%>
		        	<a href="DoctorHome.jsp?pageNum=<%= i %>">[<%= i %>]</a>
		<%		}
		        if (endPage < pageCount) {  %>
		        	<a href="DoctorHome.jsp?pageNum=<%= startPage + 10 %>">[다음]</a>
		<%		}
		    }
	}%>
   <!-- 여기에 본인 파트 종료-->
